...
app.post('/api/attendance/check', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // If it's a Check-Out, verify that they checked in today
    if (type === 'Check-Out') {
      const [checkInLogs] = await pool.query(
        "SELECT id FROM absensis WHERE karyawan_id = ? AND tipe = 'Check-In' AND waktu LIKE ? LIMIT 1",
        [req.user.karyawanId, `${formattedDate}%`]
      );
